...
META_SCHEMA = fields.Schema(
    insurer=fields.TEXT(stored=True, analyzer=custom_analyzer),
    contract_name=fields.TEXT(stored=True, analyzer=custom_analyzer),
    doc_id=fields.ID(stored=True),
)
    ...
    def _build_meta_index(self, documents: List[Document]):
        ...
        try:
            for doc in documents:
                writer.add_document(
                    doc_id=doc.doc_id,
                    contract_name=doc.meta['contract_name'] or "",
                    insurer=doc.meta['insurer'] or "",
                )
